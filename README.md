...
| 40 | `nomos-workflow-orchestration` | The workflow tier's first real execution increment: runs an ordered sequence of `WorkflowStep` declarations, each paired with a real dispatch to one of this workspace's two real backends (`nomos-agent-executor-claude-code`, `nomos-model-backend-ollama`), refusing whichever step first declares itself incoherent before that step's body ever dispatches — apart from choosing a platform. `OD-WORKFLOW-005`. |
