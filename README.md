...
| 26 | `nomos-model-package` | The first `ModelBackendPackage`/`AgentExecutorPackage` manifest maturity: identity, `PackageKind`, `PKG-007`'s first two version domains reused from `nomos-package` unchanged, and `ModelSelection` replacing the fourth. A peer of `nomos-lang-package`, not a dependent of it. |
