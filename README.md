...
| 24 | `nomos-repo-standards-document` | The shared `standards.json` acquisition step `OD-RULES-019` decided the five `nomos.cap.*.policy` providers below owe one read/parse boundary — `STANDARDS_JSON`, the `Read_To_String`/`serde_json::from_str` sequence, and one error type each of the five providers' own errors converts from rather than re-declaring. Hands back a parsed `serde_json::Value`, never a typed payload: semantic extraction of the `naming`, `limits`, `scripting`, `words` and `goals` blocks stays exactly where `OD-RULES-011` put it, in each provider's own `reading.rs`. |
...
| 25 | `nomos-repo-goals` | The one provider of `nomos.cap.goals.policy` — reads `standards.json`'s `goals`, `max_subsystems_per_goal` and `subsystems` keys through `nomos_platform::FileSystem`, alongside its four siblings for the identical reason. This workspace declares none of the three, and the reader says so rather than inventing a goal set: goal traceability is a discipline a system takes on. `OD-RULES-011`'s fifth instance. |
...
| 41 | `nomos-correction-orchestration` | The seam for correction planning and lifecycle both hosts call: `Run_Correction` composes `nomos-check-orchestration` and `nomos-corrections` into a judged, staged, validated (and optionally committed) outcome — apart from choosing a platform, walking a tree or rendering the answer. `nomos-cli`'s own `correct.rs` is a thin renderer over it, the way `gate.rs` already is over `Run_Gate`. Above `nomos-check-orchestration` (40) and `nomos-corrections` (35) so it may depend on both. `P40-CORRECTIONS-CANONICAL-SEAM`. |
| 41 | `nomos-workflow-orchestration` | The workflow tier's first real execution increment: runs an ordered sequence of `WorkflowStep` declarations, each paired with a real dispatch to one of this workspace's three real dispatch targets (`nomos-agent-executor-claude-code`, `nomos-model-backend-ollama`, `nomos-check-orchestration::Run`), refusing whichever step first declares itself incoherent before that step's body ever dispatches — apart from choosing a platform. Above `nomos-check-orchestration`'s band so `Body::Check` may depend on it. `OD-WORKFLOW-005`, `P40-WORKFLOW-CHECK-BODY`. |
| 90 | `nomos-cli` | The `nomos` binary. |
| 90 | `nomos-api` | A second real caller of `nomos-gate-orchestration`'s `Run_Gate`, `Plan` and `Explain` (all three Gate verbs), `nomos-work-orchestration`'s `Run` for all eleven `WorkCommand` verbs (`List`, `Show`, `Validate`, `Audit`, `Claim`, `Renew`, `TakeOver`, `Abandon`, `Decline`, `Finish`, `Add`), `nomos-spec-orchestration`'s `Run` for all nine `SpecCommand` verbs (`Profiles`, `Sources`, `Record`, `Table`, `Markdown`, `Freshness`, `Preview`, `Render`, `Commit`) plus `Submit`, and `nomos-correction-orchestration`'s `Run_Correction` — apart from choosing a platform or wiring an actual transport over it. |
...
| 92 | `nomos-api-transport` | The transport `nomos-api` states it does not wire: JSON-RPC 2.0 over a TCP socket, one object per line, with no JSON-RPC library and no async runtime. Serves the three Gate verbs `OD-HOST-007` admits -- `nomos.gate.plan`, `nomos.gate.run`, `nomos.gate.explain` -- and none of the twenty-one `Handle_Work_*` and `Handle_Spec_*` handlers that record excludes, because those belong to the `[repo tooling]` crates below. Not itself `[repo tooling]`: it exists precisely to answer a question an end-user repository would ask. Above band 90 because it depends on `nomos-api` and a band may not depend on its own band. Binds no listener of its own; a caller supplies one. |
| 93 | `nomos-mcp` | The MCP half of `AGT-006`'s "neutral versioned contracts and MCP tools" commitment: a real, runnable server over stdio, exposing `nomos-api-transport`'s own three Gate verbs as MCP tools (`tools/list`, `tools/call`) with a hand-written JSON Schema per tool, over the identical one-object-per-line framing that crate's own doc names as what MCP's stdio transport already uses. Depends on `nomos-api-transport` alone -- never on `nomos-api` or any orchestration crate directly -- so a `tools/call` reaches a handler only by re-serializing the call as the line that crate's own `Answer` already reads, inheriting its exclusion of every `[repo tooling]` handler rather than repeating it. Above band 92 because it depends on `nomos-api-transport`. Ships its own binary: unlike its sibling, MCP's stdio transport has no listener for a caller to bind first. |
