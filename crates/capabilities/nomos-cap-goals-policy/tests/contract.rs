//! `nomos_cap_goals_policy`'s own contract tests, compiled here rather than inside
//! `src/contract.rs` for the same reason its four siblings' are: this file can only reach
//! `nomos_capability` and `nomos_contracts` through `nomos_cap_goals_policy`'s own public
...
/// asserting it resolved which part serves which purpose, when that mapping exists nowhere
/// but the declaration and cannot be resolved from code at all.
#[test]
fn Test_The_Ceiling_Should_Refuse_A_Claim_Of_Semantic_Resolution()
{
    ...
    contract_testing::Assert_Ceiling_Refuses(
        ...
        "a provider claiming semantic resolution for a capability about a repository's own \
         declared purposes would satisfy every rule that needs it",
    );
}
...
/// The other four tests each drive [`Capability_Contract`] only to reach a property of the
/// ceiling or the registry; this checks the construction itself.
#[test]
