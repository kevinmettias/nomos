//! `nomos_cap_scripting_policy`'s own contract tests, compiled here rather than inside
//! `src/contract.rs` for the same reason its two siblings' are: this file can only reach
//! `nomos_capability` and `nomos_contracts` through `nomos_cap_scripting_policy`'s own
...
use nomos_cap_scripting_policy::{
