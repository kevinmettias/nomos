//! `nomos_cap_dependency_policy`'s own contract tests used to live inside `src/contract.rs`,
//! compiled into the crate itself — reaching `nomos_capability`'s `contract_testing` helpers
//! and `nomos_contracts`' types as an ordinary dependency that can still see this crate's own
//! private items, which is not the view a real consumer ever has. Compiled here instead, this
//! file can only reach `nomos_capability` and `nomos_contracts` through
//! `nomos_cap_dependency_policy`'s own public API — `Capability_Contract`, `Capability`,
//! `Ceiling`, `CONTRACT_VERSION` and `Payload_Schema`, all `pub` already — the same surface
//! `nomos-rules` or a registry composition root would use.

use nomos_cap_dependency_policy::{
