...
//!
//! Lives in a crate of its own, below the one provider that offers against it. Not because
//! `OD-CAPABILITY-002`'s criterion fired -- that criterion is provider contention and this
//! capability still has exactly one provider -- but because `OD-ROADMAP-005`'s fifth
//! decision required the split on a different ground: a contract a generic rule layer reads
//! should not be spelled with a vendor's crate name. `OD-CAPABILITY-017` measured the
//! bundled arrangement and found it defeated no boundary, and that measurement stands; what
//! the owner superseded was the conclusion that no split was owed yet.
//!
