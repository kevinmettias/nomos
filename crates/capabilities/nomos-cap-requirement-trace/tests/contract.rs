//! `nomos_cap_requirement_trace`'s own contract tests, compiled here rather than inside
//! `src/contract.rs` for the same reason `nomos-cap-goals-policy`'s are: this file can only
//! reach `nomos_capability` and `nomos_contracts` through `nomos_cap_requirement_trace`'s
...
use nomos_cap_requirement_trace::{
...
/// asserting it resolved something about a citation beyond whether its exact text still
/// occurs where it was named, which no provider of this capability does or could.
#[test]
