//! `nomos_cap_limits_policy`'s own contract tests, compiled here rather than inside
//! `src/contract.rs` for the same reason `nomos_cap_naming_policy`'s are: this file can
//! only reach `nomos_capability` and `nomos_contracts` through `nomos_cap_limits_
