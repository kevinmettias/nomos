...
const FINDING_TAG: &str = "clone-on-copy\t";
...
/// [`FINDING_TAG`]. Empty bytes decode to an empty, clean payload rather than being
...
mod tests
{
    ...
    {
        let payload = CloneOnCopyPayload { findings: vec![ClonedCopyType { location: "src/a.rs:1:1\nsrc/b.rs:2:2".to_owned() }] };
