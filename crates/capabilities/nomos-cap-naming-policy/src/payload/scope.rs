...
///
/// Mirrors `standards.json`'s own shape — a top-level `naming` block and a
/// `languages.<name>.naming` block that refines it — rather than flattening the two into
/// one namespace a reader could not tell apart again.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum Scope
{
    /// From the repository-wide `naming` block.
    Repository,
    /// From `languages.<name>.naming`.
    Language(String),
}
...
mod tests
{
    ...
    {
        let scope = Scope::Language("rust".to_owned());
