//! `nomos_cap_naming_policy`'s own contract tests, compiled here rather than inside
//! `src/contract.rs` for the same reason `nomos_cap_dependency_policy`'s are: this file
//! can only reach `nomos_capability` and `nomos_contracts` through `nomos_cap_naming_
//! policy`'s own public API — `Capability_Contract`, `Capability`, `Ceiling`,
//! `CONTRACT_VERSION` and `Payload_Schema`, all `pub` already — the same surface
//! `nomos-rules` or a registry composition root would use.

use nomos_cap_naming_policy::{
