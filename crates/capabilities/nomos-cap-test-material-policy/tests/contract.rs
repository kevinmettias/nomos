//! `nomos_cap_test_material_policy`'s own contract tests, compiled here rather than inside
//! `src/contract.rs` for the same reason its five siblings' are: this file can only reach
//! `nomos_capability` and `nomos_contracts` through `nomos_cap_test_material_policy`'s own
...
use nomos_cap_test_material_policy::{
...
/// The other five tests each drive [`Capability_Contract`] only to reach a property of
