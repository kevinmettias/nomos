//! `nomos_cap_dependency`'s own contract tests used to live inside `src/contract.rs`,
...
//! file can only reach `nomos_capability` and `nomos_contracts` through `nomos_cap_dependency`'s
...
use nomos_cap_dependency::{Capability, Capability_Contract, Ceiling, CONTRACT_VERSION, Payload_Schema, SCHEMA};
