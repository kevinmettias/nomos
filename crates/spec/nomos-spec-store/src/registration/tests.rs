...
fn Write(directory: &Path, name: &str, body: FixtureBody<'_>)
{
...
fn Test_A_File_That_Is_Not_A_Registration_Should_Refuse()
{
    for intruder in Intruders()
    {
        ...
        Write(&directory, "OD-GATE-001.record", FixtureBody(&format!("path: {A_REAL_RECORD}\n")));
        Write(&directory, intruder, FixtureBody("anything\n"));
...
fn Test_A_Registration_With_No_Path_Line_Should_Refuse()
{
    ...
    Write(&directory, "OD-GATE-001.record", FixtureBody("# only a comment\n\n"));
...
fn Test_A_Registration_Naming_A_Path_Outside_The_Record_Directory_Should_Refuse()
{
    for named in Paths_Outside_The_Record_Directory()
    {
        ...
        Write(&directory, "OD-GATE-001.record", FixtureBody(&format!("path: {named}\n")));
...
fn Test_Two_Registrations_Naming_One_Record_Should_Refuse()
{
    ...
    Write(&directory, "OD-GATE-001.record", FixtureBody(&format!("path: {A_REAL_RECORD}\n")));
    Write(&directory, "OD-GATE-002.record", FixtureBody(&format!("path: {A_REAL_RECORD}\n")));
...
fn Test_A_Well_Formed_Directory_Should_Be_Read()
{
    ...
    Write(&directory, "OD-GATE-001.record", FixtureBody(&format!("path: {A_REAL_RECORD}\n")));
    Write(&directory, "D-129.record", FixtureBody(&format!("path: {ANOTHER_REAL_RECORD}\n")));
...
fn Test_The_Result_Should_Be_Sorted_By_Identifier()
{
    ...
    let identifiers: Vec<String> = Read(&directory)
        .expect("Write() put both well-formed registrations in the directory made just above")
        .into_iter()
...
fn Test_Incidental_Noise_Around_The_Path_Line_Should_Not_Reach_The_Path()
{
    for (why, body) in Bodies_With_Incidental_Noise()
    {
        ...
        Write(&directory, "OD-GATE-001.record", FixtureBody(&body));

        let read = Read(&directory)
            .expect("Write() wrote this case's registration into the directory just above");
