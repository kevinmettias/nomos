...
/// The five values `Submission_Request` files under one submission.
const REQUEST_VALUE_COUNT: u32 = 5;

/// How many of `Submission_Request`'s values are unaccounted for once only `title` is supplied.
...
fn First_Failure(failures: &[nomos_spec_model::Failure]) -> &nomos_spec_model::Failure
{
...
fn Stored_Value_At(rows: &[StoredValue], index: usize) -> &StoredValue
{
...
fn Field_Value_With_Origin(field: FieldName<'_>, value: &str, origin: Origin) -> FieldValue
{
    return FieldValue {
...
fn Submission_Request(id: &str, state: SubmissionState) -> Submission
{
    return Submission {
        id: id.to_owned(),
        ...
        values: vec![
            Field_Value_With_Origin(FieldName("title"), "Items can be declined", Origin::Submitted),
            Field_Value_With_Origin(FieldName("goal"), "close superseded work", Origin::Submitted),
            Field_Value_With_Origin(FieldName("behaviour"), "a verb writes Declined", Origin::Submitted),
            Field_Value_With_Origin(FieldName("acceptance"), "it stops being claimable", Origin::Submitted),
            Field_Value_With_Origin(FieldName("invariants"), "none", Origin::Submitted),
        ],
        gaps: Vec::new(),
    };
}
...
fn Test_An_Accepted_Request_Should_Land_As_A_Node_And_Its_Values()
{
    ...
    let request = Submission_Request("FR-001", SubmissionState::Accepted);
...
{
    let mut store = Store();
    let mut submission = Submission_Request("FR-002", SubmissionState::Accepted);
    ...
    match error
    {
        AcceptError::Refused(refusal) =>
        {
            ...
            assert_eq!(First_Failure(&refusal.failures).field, "goal");
        }
...
{
    let mut store = Store();
    let mut submission = Submission_Request("FR-003", SubmissionState::Accepted);
...
{
    let mut store = Store();
    let mut submission = Submission_Request("FR-004", SubmissionState::Accepted);
    let clarified = Field_Value_With_Origin(FieldName("goal"), "what it became", Origin::Clarified);
    ...
    assert_eq!(Stored_Value_At(&rows, 0).value, "close superseded work");
    assert_eq!(Stored_Value_At(&rows, 1).value, "what it became");
    assert!(Stored_Value_At(&rows, 0).supersedes.is_none(), "the first supersedes nothing");
    assert!(Stored_Value_At(&rows, 1).supersedes.is_some(), "the second names what it superseded");
}
...
{
    let mut submission = Submission_Request(id, state);
...
fn Test_An_Open_Blocking_Gap_Should_Be_A_Row_And_Should_Refuse_Acceptance()
{
    ...
    assert_eq!(First_Failure(&refusal.failures).rule, "no-open-blocking-gap");
}
...
fn Test_A_Gap_Should_Not_Be_Closed_By_Supplying_The_Value_It_Blocks()
{
    ...
    let supplied =
        Field_Value_With_Origin(FieldName("behaviour"), "the blocked value, supplied", Origin::Submitted);
...
fn Submission_Design(id: &str, answers: CitedSubmission<'_>, state: SubmissionState) -> Submission
{
    return Submission {
        id: id.to_owned(),
        ...
        submitted_through: "cli".to_owned(),
        values: vec![
            Field_Value_With_Origin(FieldName("title"), "a design", Origin::Submitted),
            Field_Value_With_Origin(FieldName("answers"), answers.0, Origin::Submitted),
            Field_Value_With_Origin(FieldName("alternatives"), "a new verb\ndo nothing", Origin::Submitted),
            Field_Value_With_Origin(FieldName("selected"), "a new verb", Origin::Submitted),
            Field_Value_With_Origin(FieldName("architecture_delta"), "none", Origin::Submitted),
            Field_Value_With_Origin(FieldName("acceptance"), "the tests pass", Origin::Submitted),
        ],
        gaps: Vec::new(),
    };
}
...
fn Test_A_Citation_Naming_Nothing_Should_Be_Refused()
{
    ...
    let design = Submission_Design("DS-001", CitedSubmission("FR-404"), SubmissionState::Draft);
    ...
    assert_eq!(
        First_Failure(&refusal.failures).rule,
        "citation-resolves-to-an-accepted-submission"
    );
    assert!(First_Failure(&refusal.failures).remedy.contains("no submission is filed under"));
}
...
{
    let mut store = Store();
    let mut request = Submission_Request("FR-007", SubmissionState::Draft);
    Accept_Submission(&mut store, &request).expect("a draft request");
    let design = Submission_Design("DS-002", CitedSubmission("FR-007"), SubmissionState::Draft);

    let refusal = Refused_Draft_Citation(&mut store, &design);

    assert!(First_Failure(&refusal.failures).remedy.contains("accept it first"));
...
{
    let mut store = Store();
    let request = Submission_Request("FR-008", SubmissionState::Accepted);
    let design = Submission_Design("DS-003", CitedSubmission("FR-008"), SubmissionState::Draft);
    Accept_Submission(&mut store, &request)
