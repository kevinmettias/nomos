...
//!
//! I8's half — the game plans, and the rule that nothing normative rests on one alone — is the
//! section at the end of this file. The two halves share one file rather than sitting in
//! sibling modules because the derivation that counts a corpus-gated test follows the calls a
//! test makes within one file, and every one of I8's tests reaches the corpus through
//! [`Ecosystem`] or [`Archives`] above: a module of their own would hold three gated tests
//! that no declared row and no derivation could count.
...
// I8: the game plans, and the rule that nothing normative rests on one alone.
//
// These tests sit in this file rather than in a module beside it because the derivation
// that counts a corpus-gated test follows the calls a test makes within one file, and all
// three of them reach the corpus through `Ecosystem()` or `Archives()` above: a module of
// their own would hold three gated tests that no declared row and no derivation can count.
// The fixtures are this file's as well — `Ecosystem()` builds the store, `Plan_Text()`
// reads a plan, and `Counted_For()` runs the counts.
