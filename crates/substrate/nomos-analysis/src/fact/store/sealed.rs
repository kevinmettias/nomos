//! The marker that seals [`super::Store`].

/// Implemented only inside this crate, so [`super::Store`]'s invariants stay this crate's
/// to keep.
///
/// Split out of `store.rs` so the file is named for the type it declares. The trait is
