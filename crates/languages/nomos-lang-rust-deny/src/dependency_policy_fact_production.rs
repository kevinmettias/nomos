...
//! `tests/integration/tests/determinism/`, the same seam `nomos_lang_rust_clippy::
//! LintFactProduction` is discharged through, and for the same reason: a declaration
...
/// Producing the `nomos.cap.dependency.policy` fact for a workspace: running `cargo deny
/// check bans licenses sources` and encoding what it reports.
///
/// The analysis-kernel row of the domain table this crate occupies, alongside
/// `nomos_lang_rust_clippy::LintFactProduction`.
pub struct DependencyPolicyFactProduction;

impl Strategy for DependencyPolicyFactProduction
{
    /// `State`, not `StateTemporal`. `reading::Canonical_Order` sorts every violation
    /// before this provider ever encodes them, so two runs that discovered the same
    /// violations through `cargo deny`'s own JSON stream in a different order — which
    /// neither `cargo deny`'s own internal iteration nor `serde_json`'s object
    /// representation promises against — reach identical bytes. Nothing about *when* a
    /// violation was reported is part of what this fact claims; only the final, sorted set
    /// is.
