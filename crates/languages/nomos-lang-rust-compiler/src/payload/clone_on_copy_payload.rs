...
/// The whole analyzed crate's own set of clone-on-copy findings -- empty when this
/// provider's analysis found none, the same "clean is a real answer, not an absence"
/// shape `nomos_cap_dependency_policy::PolicyPayload` already has for a workspace with
/// no violations.
