...
mod tests
{
    ...
    fn Test_Check_Nested_Locks_Should_Produce_No_Finding_For_A_Clean_Fact()
    {
        ...
        let mut store = MemoryFactStore::New();
        let registry = Offered_Registry();
        Materialize(&mut store, &registry, subject, &NestedLockPayload { findings: Vec::new() });
    ...
    fn Materialize(store: &mut MemoryFactStore, registry: &Registry, subject: SubjectId, payload: &NestedLockPayload)
    {
