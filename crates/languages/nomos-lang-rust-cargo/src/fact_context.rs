...
mod tests
{
    ...
    {
        let first = Materialize_Workspace(&Repository_Root(), Context(), &StdProcessLauncher).expect("a real workspace");
        let second = Materialize_Workspace(&Repository_Root(), Context(), &StdProcessLauncher).expect("a real workspace");
