...
//!
//! Everything identity-bearing about the result is assembled here — see `nomos-lang-rust`'s own
//! `provider.rs` for the full reasoning; this module makes the identical choices for a C# file
//! in place of a Rust one.
...
///
/// The evidence is `Verified`, not `Derived`: a parser either found the declaration in the tree
/// or it did not, and this provider's own [`Assurance::Sound`](nomos_contracts::Assurance::Sound)
/// soundness claim is exactly the claim that there is no inference step in between. The
/// completeness axis is weaker and says nothing about this: a gap is not an inference.
fn Assembled_Fact(
...
/// `nomos_cap_syntax::Render_Payload`, deliberately: see that crate's own module doc for why the
/// writers stay with their providers while the reader stays shared.
#[must_use]
...
/// One observed field, in the schema's spelling. `NotObserved` — `-` — is unreachable from here
/// for the same reason it is unreachable from `nomos-lang-rust`'s encoder: this provider parses,
/// so every field it does not write is an absence it looked for.
...
mod tests
{
    ...
    /// each value carries no meaning beyond "not equal to the others".
    ...
    {
        let fact = Fact_From_Source("public class Widget { }\n");
    ...
    {
        let source = "public class Widget { public int Count { get; } }\n";
    ...
    {
        let first = Fact_From_Source("public class One { }\n");
        let second = Fact_From_Source("public class Two { }\n");
    ...
    {
        let outcome = Materialize_Syntax_Fact(Subject_Of_Path("Broken.cs"), "public class Widget { public void (", Context());
    ...
    fn Test_Encode_Payload_Should_Be_Stable_And_Not_Empty()
    {
        ...
        assert_eq!(
            rendered,
            "unexpanded\t0\n\
             item\t0\tUsing\tNotApplicable\tSystem\t.\t.\n\
             item\t1\tClass\tPublic\tWidget\t.\t.\n\
             item\t2\tField\tPrivate\tWidget::slots\t.\t+value\n\
             item\t3\tFunction\tPublic\tWidget::Reset\t.\t+fn/0\n"
        );
        assert!(!rendered.contains('\r'), "line endings must not be local");
    }

    /// The header this provider has something real to put in, encoded.
    #[test]
    fn Test_The_Encoding_Should_Carry_The_Regions_This_Provider_Declined()
    {
        let fact = Fact_From_Source(
            "public class Widget\n\
             {\n\
    ...
    /// What this provider writes is what the schema says a payload is — decoded here through the
    /// shared reader rather than compared against a sibling provider's own bytes, for the reason
    /// `nomos_cap_syntax`'s module doc gives.
    #[test]
    fn Test_This_Providers_Payload_Should_Decode_Under_The_Schemas_Own_Reader()
    {
        let fact = Fact_From_Source("using System;\npublic class Widget\n{\n    public void Reset() { }\n}\n");
        ...
        assert_eq!(third.qualified_name, "Widget::Reset");
        assert_eq!(third.Own_Name(), "Reset");
        assert!(third.Is_Public());
    }

    /// The mark the schema reserves, and the value that is deliberately not it: a `using`
    /// declares no accessibility at all, while a method that simply states none could have.
    #[test]
    fn Test_The_Two_Absences_Should_Decode_Apart_Under_The_Shared_Reader()
    {
        let fact = Fact_From_Source("using System;\npublic class Widget\n{\n    void Reset() { }\n}\n");
        ...
        let directive = payload
            .items
            .iter()
            .find(|item| return item.Own_Name() == "System")
            .expect("the using directive is an item");
        let method = payload
            .items
            .iter()
            .find(|item| return item.Own_Name() == "Reset")
            .expect("the method is an item");

        assert!(directive.Declares_No_Visibility(), "{directive:?}");
        assert!(!method.Declares_No_Visibility(), "{method:?}");
        assert!(!method.Is_Public(), "{method:?}");
    }
    ...
    {
        return match Materialize_Syntax_Fact(Subject_Of_Path("Widget.cs"), source, Context())
        {
            Materialization::Materialized(fact) => *fact,
            // Every source passed to this helper is C# its author wrote to be parseable, so a
            // refusal is a broken fixture and not a reading worth handing back to the tests
            // below, which compare two facts to each other and would pass vacuously if a
            // provider that had begun refusing everything made both sides equally absent. The
            // parser's own message is printed because it names what stopped it.
