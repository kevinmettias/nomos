...
//!
//! A deliberate twin of `nomos-lsp::build_variant::Host_Variant` and
//! `nomos-cli::check::composition::Host_Variant`: `env!` resolves against the crate that
//! calls it, so a build variant read from inside `nomos-check-orchestration` would describe
//! that library's own compilation rather than this one's. Each real caller of the seam
//! carries this one function for itself; see this crate's own `build.rs` for why the value
//! cannot be shared.
