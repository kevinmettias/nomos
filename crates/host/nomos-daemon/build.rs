//! What this crate is actually being built as, passed through to the crate.
//!
//! A deliberate twin of `nomos-lsp`'s, `nomos-cli`'s and `tests/integration`'s own, captured
//! identically here for the identical reason: a build script may not depend on a crate, so a
//! `BuildVariant` computed by one cannot be shared with another. This crate needs its own
//! copy because it calls `nomos_check_orchestration::Run_Reassessing` directly, the same
//! caller-supplies-the-variant contract every other real caller of that seam carries.
