...
mod tests
{
    ...
    /// must not be descended into, the same as `target` already is not. A real, already
    /// merged leftover worktree under `.claude/worktrees/` contributed dozens of stale
    /// findings against its own checkout before this test existed.
    #[test]
    fn Test_Read_Sources_Should_Not_Descend_Into_A_Nested_Git_Worktree()
    {
        let root = Fresh_Root("nomos-cli-check-sources-nested-worktree");
        std::fs::write(root.join("a.rs"), "pub fn One() {}\n").expect("writable");
