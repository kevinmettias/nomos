...
//!
//! A deliberate twin of `crates/host/nomos-cli/build.rs`, not a shared dependency of it -- a
//! build script may not depend on a crate, so the two cannot be one. See that file's own doc
//! for why each of the four values is captured here rather than derived from
//! `std::env::consts` or left unstated.
