...
///
/// Follows [`crate::spec::record::Handle_Spec_Record`]'s own composition: builds a
/// `CorpusRequest` from the environment, dispatches through the shared `Run` entry point,
/// matches `SpecOutcome::Table`. `run::table::Table` itself never touches a `FileSystem`, the
/// same as `Record`.
#[must_use]
