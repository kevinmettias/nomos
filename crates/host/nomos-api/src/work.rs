...
//! "one verb at a time, not the whole command set" scope `Handle_Gate_Run` already uses for
//! Gate's own `run`.

use nomos_ledger::{
    AddRefusal, Claim_Refusal, ClaimRefusal, FileLedger, FinishRefusal, ItemId, ItemState, LedgerDocument, LedgerItem,
    Reservation, Territory, VerificationRecord,
};
...
/// [`WorkReservationResponse`]/[`WorkAbandonResponse`]/[`WorkDeclineResponse`]/
/// [`WorkFinishResponse`] each add to theirs: `nomos_ledger::AddRefusal` exposes no single
/// `Is_X`-style method the way `ClaimRefusal`/`FinishRefusal` do -- `crates/host/nomos-cli/
/// src/work/report.rs`'s own `Code_For_Refusal` maps its six variants across three different
/// exit codes, and nothing here reduces that to one bit without becoming a second,
/// uncanonical implementation of a distinction only that function currently draws.
#[derive(Debug, Serialize)]
#[serde(tag = "outcome", rename_all = "snake_case")]
...
{
    fn From(result: Result<(), AddRefusal>) -> Self
    {
        return match result
        {
            Ok(()) => Self::Added,
...
/// not a shared dependency of it, the same "a walk is a composition-root concern" division
/// `sources.rs` already documents for Gate's own walk. `nomos_platform::FileSystem` has no
/// directory-listing operation, so `nomos_work_orchestration::Run`'s own `published` closure
/// exists precisely so each composition root can answer this its own way.
///
