//! The first Go `LanguagePackage` manifest shape.

use crate::GoVersion;
use nomos_contracts::{PackageId, PackageKind};
use nomos_package::{PackageVersion, ProtocolRange, ProviderRegistration};

/// A `LanguagePackage`: registration and translation boundary for the Go ecosystem, per
/// `ARCH-001` — the identical shape `nomos_lang_package::LanguagePackage` carries for
/// Rust, scoped to the same first manifest maturity `OD-PACKAGE-001` asked for and no
/// further.
...
pub struct LanguagePackage
{
    ...
    /// The Go versions this package recognizes. `PKG-007`'s third version domain. Never
    /// empty: a `LanguagePackage` that recognizes no language version recognizes nothing.
    pub language_versions: Vec<GoVersion>,
