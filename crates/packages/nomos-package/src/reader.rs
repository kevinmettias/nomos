//! Reading a manifest from JSON, and refusing what it cannot resolve.
//!
//! This is `OD-PACKAGE-001`'s second step, generalized by `OD-PACKAGE-007`: it is written
//! in the discipline the former record names -- `nomos-spec-store`'s registration reader
...
//! `LanguagePackage`, a version domain that will not parse, a provider the caller's
//! allowlist does not name -- each is a named [`ManifestError`] variant, and none of
//! them is reached by defaulting a field or by taking the first value found and ignoring
//! a conflict.
//!
//! `known_providers` is a parameter rather than a constant, unlike
//! `nomos-lang-package`'s own `Parse_Manifest`: this crate does not know which providers
//! any given language's package may register, and inventing a workspace-wide list here
...
/// manifest that were silently repaired or dropped a field would be a manifest that
/// quietly claims less, or something different, than what is on disk.
...
pub enum ManifestError
{
    ...
    /// A registered provider is not one the caller's allowlist names.
    UnresolvedProvider
    {
        at: String,
        provider: String,
...
/// Reads and resolves a manifest from a file, refusing a `providers` entry not named in
/// `known_providers`.
///
/// # Errors
///
/// Returns [`ManifestError`] on every malformed input; see the enum for the named cases.
pub fn Read_Manifest(path: &Path, known_providers: &[&str]) -> Result<PackageManifest, ManifestError>
{
    ...
    return Parse_Manifest(&text, &at, known_providers);
}

/// Parses a manifest from its text, naming `at` in every refusal and refusing a
/// `providers` entry not named in `known_providers`.
///
...
) -> Result<PackageManifest, ManifestError>
{
    ...
    let providers = Providers_Field(root, at, known_providers)?;
...
/// `providers`, resolved to at least one [`ProviderRegistration`], each naming a
/// provider `known_providers` carries.
fn Providers_Field(
    object: &Map<String, Value>,
    at: &str,
    known_providers: &[&str],
) -> Result<Vec<ProviderRegistration>, ManifestError>
{
