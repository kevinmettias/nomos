...
impl ValidatedPlan
{
    ...
    /// the change.
    pub fn Commit(self, live: &mut Workspace) -> Result<CommittedPlan, CorrectionError>
    {
        Assert_Not_Moved(self.base, live)?;

        live.Apply(&self.forward)?;

        return Ok(CommittedPlan::Of(self.base, live.Id(), self.reverse));
    }
}
...
mod tests
{
    ...
    fn Test_Committing_A_Validated_Plan_Should_Change_The_Workspace()
    {
        ...
        let committed = validated.Commit(&mut base).expect("commits cleanly");
