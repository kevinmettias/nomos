...
///
/// Replaces the two variants this enum used to carry, `ClaudeCode(TaskEnvelope)` and
/// `Ollama(TaskEnvelope)`. Those made the variant a step was written as its backend choice:
/// dispatch matched the variant straight to an executor crate, so nothing resolved anything
/// and a step could not express "whatever answers to this family" at all.
///
