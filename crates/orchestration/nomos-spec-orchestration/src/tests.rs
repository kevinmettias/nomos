...
use crate::outcome::{
    FreshnessRefusal, PreviewRefusal, RecordRefusal, RenderRefusal, Reproduction, SpecOutcome, SubmitRefusal,
    TableRefusal, Verdict,
};
use crate::request::{
...
fn Test_Submit_Should_Accept_A_Complete_Submission_With_Submitted_Origin()
{
    ...
    let answer = Submit(&mut assembly, &Complete_Feature_Request("FR-ORCH-001", None), &StdFileSystem)
...
fn Test_Submit_Should_Place_Its_Subject_Dossier_Projection_When_Into_Is_Given()
{
    ...
    let answer = Submit(&mut assembly, &Complete_Feature_Request("FR-ORCH-002", Some(into)), &StdFileSystem)
        .expect("a complete feature request is accepted");
