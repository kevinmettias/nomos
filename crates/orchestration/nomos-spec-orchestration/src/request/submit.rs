...
///
/// Moved verbatim from `nomos-cli::request`'s own `SubmitRequest`, alongside the [`Constructed`]
/// conversion `run::submit` now carries -- see that module for why `state` and
/// `contract_version` are facts about this run rather than about the submission, and why every
/// `field` value carries origin `submitted` unconditionally.
///
/// [`Constructed`]: crate::run::submit::Constructed
