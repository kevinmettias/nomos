...
/// feeds [`Disposition_Of_Findings`] rather than adding a variant of their own; `ARC-ROADMAP-001`'s
...
pub enum GateRunOutcome
{
    ...
    /// no source or no fact was found to judge. [`Disposition_Of_Findings`] never returns this
    /// variant; [`crate::Run_Gate`] assigns it directly for a [`nomos_check_orchestration::
    /// CheckOutcome`] that never reached `Judged`, the one condition `Disposition_Of_Findings`
    /// itself cannot see because it reduces a list of findings, not the outcome that produced
    /// one.
    Indeterminate,
}
...
/// not gated on.
#[must_use]
pub fn Disposition_Of_Findings(findings: &[Finding]) -> GateRunOutcome
{
