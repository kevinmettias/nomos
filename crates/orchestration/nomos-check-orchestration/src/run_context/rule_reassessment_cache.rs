...
/// produced by rules that performed no read in the call it is looking at, so a trail that
/// lived only for the call would be empty for nearly every finding it actually shows.
/// Reusing the trail with the findings it belongs to is what stops the mechanism from being
/// worst exactly where it is wanted most.
#[derive(Default)]
...
impl RuleReassessmentCache
{
    ...
    /// Records what a real invocation of `rule` found and what that same invocation read,
    /// for [`Self::Reusable`] and [`Self::Supporting_Facts`] to read back on a later call.
...
mod tests
{
    ...
    fn Test_Every_Source_Judging_Rule_Should_Be_Reassessed_When_The_Syntax_Family_Changed()
    {
        ...
        for descriptor in DESCRIPTORS.iter().filter(|descriptor| return Judges_A_Source(descriptor))
        {
            ...
            cache.Record(descriptor.id, Vec::new(), Vec::new());
    ...
    fn Test_A_Workspace_Rule_Should_Stay_Reusable_Across_A_Source_Text_Change()
    {
        ...
        let mut cache = RuleReassessmentCache::New();
        cache.Record(workspace.id, Vec::new(), Vec::new());
