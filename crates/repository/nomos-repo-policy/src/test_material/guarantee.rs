...
use nomos_cap_test_material_policy::{Capability, CONTRACT_VERSION};
use nomos_capability::ProviderOffer;
use nomos_contracts::{Assurance, FactVariant, Guarantee, IncrementalGranularity};

/// This provider's own name.
///
/// Named for what it reads, the same way `nomos.repo.architecture` is named for
/// `nomos-architecture.json` rather than for a hypothetical second reader of the identical
/// file.
pub const PROVIDER: &str = "nomos.repo.test-material";

/// What this provider claims, on every axis.
///
/// [`FactVariant::Syntactic`] at the ceiling, matching
/// [`nomos_cap_test_material_policy::Ceiling`] exactly: this provider reads
/// `nomos-test-material.json`'s own JSON structure with no name resolution and no inference
/// over it. There is nothing here it could resolve — where a repository files its fixtures
/// exists nowhere but the declaration.
///
...
///
/// [`IncrementalGranularity::WholeWorkspace`]: one repository, one
/// `nomos-test-material.json`, one declared fixture-location set.
#[must_use]
...
/// This provider's offer against [`nomos_cap_test_material_policy::Capability_Contract`].
#[must_use]
...
mod tests
{
    ...
    {
        use nomos_cap_test_material_policy::Ceiling;
    ...
    {
        use nomos_cap_test_material_policy::Capability_Contract;
