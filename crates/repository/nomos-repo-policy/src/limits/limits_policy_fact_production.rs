...
//! `tests/integration/tests/determinism/`, the same seam `nomos_lang_rust_deny::
//! DependencyPolicyFactProduction` is discharged through, and for the same reason: a
...
/// Producing the `nomos.cap.limits.policy` fact for a workspace: reading `standards.json`'s
/// `limits` block and encoding what it declares.
///
...
pub struct LimitsPolicyFactProduction;

impl Strategy for LimitsPolicyFactProduction
{
    /// `State`, not `StateTemporal`. `reading::Canonical_Order` sorts every row by
    /// `(scope, key)` before this provider ever encodes them, so two runs that read
    ...
    /// Thresholds` already exercises implicitly and what this declaration commits to
    ...
    /// `BitIdentical`. The output is bytes, encoded by `nomos_cap_limits_policy::
