...
use nomos_cap_scripting_policy::{Capability, CONTRACT_VERSION};
use nomos_capability::ProviderOffer;
use nomos_contracts::{Assurance, FactVariant, Guarantee, IncrementalGranularity};

/// This provider's own name.
///
/// Named for what it reads, the same way `nomos.repo.standards` and `nomos.repo.limits`
/// are each named for the file they read rather than for a hypothetical second reader of
/// the identical file.
pub const PROVIDER: &str = "nomos.repo.scripting";

/// What this provider claims, on every axis.
///
/// [`FactVariant::Syntactic`] at the ceiling, matching [`nomos_cap_scripting_policy::
/// Ceiling`] exactly: this provider reads `standards.json`'s own JSON structure with no
/// name resolution and no inference over it.
///
...
/// This provider's offer against [`nomos_cap_scripting_policy::Capability_Contract`].
#[must_use]
...
mod tests
{
    ...
    {
        use nomos_cap_scripting_policy::Ceiling;
    ...
    {
        use nomos_cap_scripting_policy::Capability_Contract;
