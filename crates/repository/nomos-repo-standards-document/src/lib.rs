//! Band 24 -- the shared acquisition step beneath the five `nomos.cap.*.policy` providers.
//!
//! `OD-RULES-019` measured `nomos-repo-standards`, `nomos-repo-limits`, `nomos-repo-
//! scripting`, `nomos-repo-words` and `nomos-repo-goals` each independently declaring the
//! same `STANDARDS_JSON` constant, the same `Read_To_String`/`serde_json::from_str`
//! sequence, and a byte-for-byte identical `{ reason: String }` error struct renamed per
//! crate, and decided the physical read/parse step beneath the five is worth sharing while
//! the five capability contracts and providers stay exactly as separate as `OD-RULES-011`
//! built them.
//!
//! This crate owns only that physical step. [`Read_Standards_Document`] hands back a
...
//! `reading.rs`, unchanged by this crate's existence. An absent file is not an error: it
