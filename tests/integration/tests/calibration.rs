...
fn Verdicts() -> Vec<RuleVerdict>
{
    return vec![
        ...
        RuleVerdict { id: "abbreviations", expected: Expected::NotAllDeserved(2), reason: "2 findings, 1 deserved. `val` (the real, private function fn val(c, idx) -> Result<u8, FromHexError>) is a genuine, deserved true positive: DEFAULT_BANNED_WORDS lists \"val\" verbatim (ported from code-standards' own defaults.go) and hex's author really did choose that terse name. `alloc` is NOT deserved: it is the name of `extern crate alloc;`, a declaration whose name is fixed by the real crate being linked (identical in kind to the already-carved-out `use`-binding exemption -- \"a name chosen wherever the binding's target was declared, not here\"), but Is_Exempt in abbreviations.rs only recognizes ItemKind::Use, not ItemKind::ExternCrate, so this name nobody here chose is judged anyway. A genuine, precise, newly-discovered gap, invisible from nomos's own tree (edition 2021, no `extern crate` statements) -- open item for the calling session: extend Is_Exempt to ItemKind::ExternCrate the same way it already covers Use." },
